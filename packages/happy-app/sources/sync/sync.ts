...
import { apiFetch, hedgedApiFetch } from './apiFetch';
import { log } from '@/log';
...
class Sync {
    ...
    async sendMessage(
        ...
        try {
            const API_ENDPOINT = getServerUrl();
            // Hedged send: latency-tolerant on flaky networks; localId dedup keeps it safe.
            const response = await hedgedApiFetch(
